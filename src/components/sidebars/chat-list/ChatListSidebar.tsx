...
const ChatListSidebar = memo(() => {
  const [isPersonalSidebarOpen, setPersonalSidebarOpen] = useState(false);
  ...
  const onPersonalSidebarOpen = () => {
    setPersonalSidebarOpen(!isPersonalSidebarOpen);
  };
  const logout = () => {
    dispatch(setToken(""));
    navigate("/signin");
  ...
  return (
    <div className="col-[1] row-[1] flex h-full w-full flex-col items-center overflow-y-auto bg-slate-100 p-1 pt-0 dark:bg-slate-900">
        ...
        <>
          ...
          >
            ...
            >
              ...
              <label onClick={onPersonalSidebarOpen}>
                {" "}
                <p>Settings</p>
              </label>
            </div>
      ...
      <PersonalInfoSidebar
        isOpen={isPersonalSidebarOpen}
        setOpen={onPersonalSidebarOpen}
      />
    </div>
  );
});
